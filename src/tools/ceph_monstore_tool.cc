...
int update_osdmap(MonitorDBStore& store, version_t ver, bool copy,
  ...
  t->put(prefix, store.combine_strings("full", dst), bl);
...
int rewrite_transaction(MonitorDBStore& store, int version,
  ...
  // prepare a transaction to rewrite the epoches
  ...
  for (version_t v = good_version + 1; v <= last_committed; v++) {
    cout << "rewriting commit #" << v << "/" << last_committed << std::endl;
    r = update_osdmap(store, v, false, crush, t);
    if (r)
      return r;
  }

  // add a new osdmap epoch to store so monitors will update its current osdmap
  // in addition to the ones stored in epoches.
  MonitorDBStore::TransactionRef new_osdmap(new MonitorDBStore::Transaction);
  r = update_osdmap(store, last_committed++, true, crush, new_osdmap.get());
  if (r)
    return r;
  cout << "adding a new commit #" << last_committed << std::endl;
  new_osdmap->put(prefix, "last_committed", last_committed);
  new_osdmap->put(prefix, "full_lastest", last_committed);
  r = store.apply_transaction(new_osdmap);
  if (r)
    return r;
  // and add this change to the transaction also
  t->append(new_osdmap);
  return 0;
}

/**
 * create a new paxos version which carries a proposal to rewrite all epoches
 * of incremental and full map of "osdmap" after a faulty crush map is injected.
 * so the leader will trigger a recovery and propogate this fix to its peons,
 * after the proposal is accepted, and the transaction in it is applied. all
 * monitors will forget the bad crush map, and have a new osdmap epoch with the
 * good crush map in it.
 */
int rewrite_crush(const char* progname,
  ...
  op_desc.add_options()
    ...
    ("good-epoch", po::value<int>(&version),
     ...
     "good chance that we have more epoches slipped into the monstore after "
  ...
  version_t pending_pn = (store.get(prefix, "accepted_pn") / 100 + 3) * 100 + 99;
