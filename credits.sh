...
git log --no-merges --pretty='%aN <%aE>' $range --since=$start_time --until=$end_time | git -c mailmap.file=.peoplemap check-mailmap --stdin | sort | uniq -c | sort -rn | nl | grep $4

echo
echo "Commits, by organizations"
git log --no-merges --pretty='%aN <%aE>' $range --since=$start_time --until=$end_time | git -c mailmap.file=.organizationmap check-mailmap --stdin | sort | uniq -c | sort -rn | nl | grep $4
echo
